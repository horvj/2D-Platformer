...
func _physics_process(delta):
	if Global.player1Input == "Swordsman":
	...
	# Add the gravity.
		...
		if Input.is_action_just_pressed("Attack3"):
			...
			currentAbility = 2
			attack()
	if Global.player2Input == "Swordsman":
...
func damage(d):
	health -= d
	if health <= d and Global.player1Input == "Swordsman":
		get_tree().change_scene_to_file("res://UI/Player2Win.tscn")
		queue_free()
	elif health <= d and Global.player2Input == "Swordsman":
